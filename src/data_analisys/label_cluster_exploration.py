    ...
    for method in LINK_METHODS:
        ...
        for type_ in TYPES:
            ...
            try:
                ...
                raise ValueError
                # data = pd.read_csv(f'{PROCESSED_DATA_FOLDER}/corrected.csv', index_col=0)
                # mat = data.to_numpy()
                # q75, q25 = np.percentile(mat, [75 ,25],axis=1,keepdims=True)
                # iqr = q75 - q25
                # norm = (mat - np.median(mat,axis=1,keepdims=True))/iqr
                # data_df = pd.DataFrame(norm, columns=data.columns,index=data.index)

                # data_df.to_csv(f'{PROCESSED_DATA_FOLDER}/2_way_norm.csv')
            ...
            study_map = list(map(get_study,data_df.columns))
            # raise ValueError("Running in the cluster")
            d = dict([(y,x+1) for x,y in enumerate(sorted(set(study_map)))])
            batches = []
            for el in study_map:
                batches.append(d[el])
            # sample_temp = list(data_df.T.index.map(lambda x: x.split('_')[0]))
            # study_sample_map = zip(study_map,sample_temp)

            new_col = list(map(lambda x : x.split('_')[-1],data_df.columns))
            ...
            #SORT DF
            listings = list(data_df.columns)
            listings.sort()
            data_df = data_df[listings]
            sample_index = list(data_df.columns)
            ...
            sil_scores = list(map(lambda x: {x:silhouette_score(data_df.T.to_numpy(),labels=maps[x])}, maps))
            ...
            test = plot_heat_map(data_df,figure_out_path,cluster=False,col_cluster=True,typ='png',title = 'overview overview study',log_norm=True, col=color_df, name='general_overview')
            # del scores['key']
            # del scores['key']
