from diff_exp_and_enrichment.diff_expr import diff_exp_combine_tissues,diff_exp
import pandas as pd
from diff_exp_and_enrichment.pr_rank_gene_enrich import get_go_data,perform_gsea_enrichment
from diff_exp_and_enrichment.plot_enrichment import plot_enrichment_scatter_interactive
import numpy as np

def run_diff_exp_and_enrichment(save_dir:str='/home/alex/Documents/GitHub/Data_collection/df_final',data_types = ['2_way_norm', 'imputed', 'corrected'],pures = [True],
                                Fulls = [True,False],filter_low_combination = [15,10,0],tissues = [None,'leaf']):    
    for fil in filter_low_combination:
        for data_type in data_types:
            for pure in pures:
                for Full in Fulls:
                    for tissue in tissues:
                        exp_name = f'1.0_{tissue if tissue else 'All_tissues'}_{'full' if Full else 'sanity'}_{'pure' if pure else 'mixed'}_min_group_{fil}'
                        treatments = [
                            "Drought Stress",
                            "Salinity Stress",
                            "Heat Stress",
                        ...
                        # if tissues:
                        #     diff_exp(treatments=treatments,tissues=tissues,save_dir=save_dir,data_type=data_type,exp_name=exp_name)
                        # else:
                        diff_exp_combine_tissues(treatments,save_dir,data_type,exp_name,samples=samples_GSE41935,pure=pure,tissue=tissue,filter_low_combination=fil)
                        ...
                        out_path = f'GSEA_enrichment_results/GSEA_enrichment_{data_type}_{exp_name}/'
                        ...
                        for stress in treatments:
                            try:
                                ...
                                diff_exp_results = pd.read_csv(f'diff_exp/{exp_name}_{data_type}/{tissue if tissue is not None else 'All-Tissues'}_{stress}_genes.csv') #TODO: this is hardcoded from {output_filename} in {diff_exp_combine_tissues and diff_exp}
                                ...
                                plot_enrichment_scatter_interactive(gsea_results_df,save_path=f'plots_enrichment/{exp_name.split('_')[0]}/{'full' if Full else 'sanity'}/{tissue if tissue is not None else 'All-Tissues'}/{data_type}/{fil}/{'pure' if pure else 'mixed'}/{stress}.html',
                                                                    title=f'GSEA for {stress} on {tissue if tissue is not None else 'All-Tissues'} on {'full' if Full else 'sanity'} with {'pure' if pure else 'mixed'} treatments with a filter of >{fil}')
